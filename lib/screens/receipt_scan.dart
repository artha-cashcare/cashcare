...
import 'package:cashcare/services/receipt_service.dart';
...
class ReceiptScanPage extends StatefulWidget {
  ...
  String? _extractTotalAmount(String text) {
    ...
    final numberOnlyRegex = RegExp(r'^\d{1,3}(?:,\d{3})*(?:\.\d{2})?$');
  ...
  String? _extractDate(String text) {
    final datePatterns = [
      RegExp(r'\b(Transaction Date:|BillDate:)\s*(\d{4}[-/.]\d{2}[-/.]\d{2})\b'),
      RegExp(r'\b\d{4}[-/.]\d{2}[-/.]\d{2}\b'),
      RegExp(r'\b\d{2}[-/.]\d{2}[-/.]\d{4}\b'),
      RegExp(r'\b\d{2}[-/.]\d{2}[-/.]\d{2}\b'),
      RegExp(r'\b\d{1,2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*[,]? \d{4}\b', caseSensitive: false),
      ...
      if (match != null) {
        try {
          ...
          if (dateText != null) {
            ...
            return DateFormat('yyyy-MM-dd').format(extractedDate);
          }
        } catch (e) {
          continue;
        }
  ...
  Future<void> _submitReceipt() async {
    ...
    if (_image == null) {
      _showErrorSnackbar('Please select a receipt image');
      return;
    }
    ...
    try {
      final success = await ReceiptService().uploadReceipt(
        receiptImage: _image!,
